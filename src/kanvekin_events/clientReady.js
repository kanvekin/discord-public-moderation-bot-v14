...
const tasks = require("../kanvekin_schemas/tasksSchema")
...
let messageTop = [];
let voiceTop = [];
const messageUsersData = await messageUser.find({ guildId: kanvekin_config.guildID }).sort({ topStat: -1 });
const voiceUsersData = await voiceUser.find({ guildId: kanvekin_config.guildID }).sort({ topStat: -1 });
messageUsersData.filter(kanvekin => client.guilds.cache.get(kanvekin_config.guildID).members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => { messageTop.push({ top: index + 1, avatar: client.guilds.cache.get(kanvekin_config.guildID).members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: client.guilds.cache.get(kanvekin_config.guildID).members.cache.get(x.userId).user.tag, score: `${Number(x.topStat).toLocaleString()}` }) })
voiceUsersData.filter(kanvekin => client.guilds.cache.get(kanvekin_config.guildID).members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => voiceTop.push({ top: index + 1, avatar: client.guilds.cache.get(kanvekin_config.guildID).members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: client.guilds.cache.get(kanvekin_config.guildID).members.cache.get(x.userId).user.tag, score: `${moment.duration(x.topStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))
...
mesaj.then(mr => {
if(mr){mr.edit({content:`> **${client.emoji("emote_chat") !== null ? client.emoji("emote_chat"):"💭"} Mesaj Sıralaması <t:${Math.floor(Date.now()/1000)}:R> Güncellendi!**`,files:[{attachment:messageTopCanvas,name:"kanvekin-was-here.png"}]}).catch(err => { })
}});
}
if (voiceTop.length > 0) {
if(!ses || ses == undefined)return
ses.then(mr => {
if(mr){mr.edit({content:`> **${client.emoji("emote_voice") !== null ? client.emoji("emote_voice"):"🔉"} Ses Sıralaması <t:${Math.floor(Date.now()/1000)}:R> Güncellendi!**`,files:[{attachment:voiceTopCanvas,name:"kanvekin-was-here.png"}]}).catch(err => { })
}});
}
}, 30000);
    const bannedTag = client.commands.find(kanvekin => kanvekin.name == "yasaklıtag");
    if(bannedTag){setInterval(() => {bannedTag.kanvekins(client,client.guilds.cache.get(kanvekin_config.guildID))}, 5000)}
