...
const guildChannel = require("../kanvekin_schemas/messageGuildChannelsSchema");
const userChannel = require("../kanvekin_schemas/messageChannelsSchema");
const { Events } = require("discord.js");
const point = require("../kanvekin_schemas/staffsSchema");
const client = global.client;
const nums = new Map();
module.exports = async (message) => {
const prefix = message.content.toLowerCase().startsWith(kanvekin_config.prefix);
if (message.author.bot || !message.guild || prefix || message.content == null) return;

	if (kanvekin_config.taskSystem && kanvekin_config.staffs.some((x) => message.member.roles.cache.has(x))) {
