...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if (message.member.permissions.has(PermissionFlagsBits.Administrator)) {
            ...
            message.reply({ embeds: [kanvekin_embed.setDescription(`> **Başarıyla Kullanıcıyı Kanalınıza Çektiniz!**`).setColor("#00ff00")] }).sil(5);
            client.true(message)
        } else {
            mesajkanvekin = message.reply({ content: `> **${member}, ${message.author} \`${member.voice.channel.name}\` Adlı Kanala Sizi Çekmek İstiyor, Kabul Ediyomusun?**`, components: [kanvekin_dropdown] })
