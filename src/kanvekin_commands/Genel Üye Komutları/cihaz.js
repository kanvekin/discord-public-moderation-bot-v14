...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if (user.presence == null) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Belirtilen User Ofline Durumda Olduğu İçin Kontrol Edilemiyor!**`)] }).sil(5);
        ...
        message.reply({ embeds: [kanvekin_embed.setDescription(`> **${user} Kullanıcısının Aktif Cihazları!**\n**Durum; \`${tür2[user.presence.status]}\`**\n**Cihazlar; ${dev.map(x => `\`${tür[x]}\``).join("\n")}**`).setThumbnail(user.user.avatarURL({dynamic:true}))] });
    }
}
