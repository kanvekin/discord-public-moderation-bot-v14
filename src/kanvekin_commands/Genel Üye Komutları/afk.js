...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        return message.channel.send({ embeds: [kanvekin_embed.setDescription(`> **${message.member} "${["www", ".com", "discord.gg", ".gg", "discord"].some(kanvekin => reason.includes(kanvekin)) ? `(Reklam)` : reason}" Sebebiyle AFK Moduna Giriş Yaptın!**`)] }).sil(5);
    }
}
...
client.on(Events.MessageCreate, async (message) => {
    ...
    if (message.mentions.users.first()) {
        ...
        if (!data) return;
        return message.channel.send({ embeds: [new EmbedBuilder().setDescription(`> **${message.mentions.users.first()} "${["www", ".com", "discord.gg", ".gg", "discord"].some(kanvekin => data.reason.includes(kanvekin)) ? `(Reklam)` : data.reason}" Sebebiyle <t:${Math.floor(data.time / 1000)}:R> AFK Moduna Geçiş Yaptı!**`).setColor("Random")] }).sil(8);
    }else 
