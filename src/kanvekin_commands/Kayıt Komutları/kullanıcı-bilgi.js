...
module.exports = {
    ...
    usage:"kullanıcı-bilgi [@kanvekin / ID]",
    category:"kayıt",
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if (member && member.presence && member.presence.activities && member.presence.activities.some(kanvekin => kanvekin.name == "Spotify" && kanvekin.type == ActivityType.Listening)) {
          ...
          return message.reply({files:[{name:"canvafy.png",attachment:spotify}],embeds: [kanvekin_embed.setDescription(
            `**
            ...
            `).setThumbnail(member.user.displayAvatarURL({dynamic:true})).setTitle(`${member.user.tag} Bilgileri`).setImage('attachment://canvafy.png').setURL(`https://discord.gg/82`)] });
        }

       return message.reply({embeds: [kanvekin_embed.setDescription(
      `**
      • Kullanıcı: (<@${member.id}> - \`${member.id}\`) (${member.roles.highest})
      • Sunucuya Katılım Sırası: ${(message.guild.members.cache.filter(a => a.joinedTimestamp <= member.joinedTimestamp).size).toLocaleString()}/${(message.guild.memberCount).toLocaleString()}
      • Hesap Kuruluş: <t:${Math.floor(member.user.createdTimestamp / 1000)}> (<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>)
      ...
      • Rolleri: ${(member.roles.cache.filter(a => a.name !== '@everyone').map(a => a).join(' ') ? member.roles.cache.filter(a => a.name !== '@everyone').map(a => a).join(', ') : 'Üzerinde Hiç Rol Bulunmamakta!')}
      • Avatar: [Tıkla](${member.user.displayAvatarURL({dynamic:true})})**
      `).setThumbnail(member.user.displayAvatarURL({dynamic:true})).setTitle(`${member.user.tag} Bilgileri`).setImage(message.guild.bannerURL({dynamic:true,size: 4096})).setURL(`https://discord.gg/82`)] });
    }
}
