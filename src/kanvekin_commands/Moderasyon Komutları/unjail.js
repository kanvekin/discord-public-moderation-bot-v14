...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if (limitData >= kanvekin_config.limit.jailLimit)return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Cezalıya Atma Limitin Dolmuş Durumda, Lütfen Sonra Tekrar Dene!**`)]}).sil(5);
        if (reason.length < 1)return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Geçerli Bir Sebep Belirt!**`)]}).sil(5);
        if (!member.roles.cache.has(jailRoles[0]))return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Belirtilen User Zaten Cezalıda!**`)]}).sil(5);
