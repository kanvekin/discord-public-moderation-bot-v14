...
module.exports = {
    name: "rol",
    usage: "rol ver / al [@kanvekin / ID] [@Rol / ID]",
    category:"rol",
    aliases: ["rol-işlem", "r"],
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if (!args[0]) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Eksik Argüman Girildi!**\n> \`${kanvekin_config.prefix}rol ver / al @User @Role\``)] }).sil(5);
        if (args[0] != "al") {
            if (args[0] != "ver") {
                return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Eksik Argüman Girildi!**\n> \`${kanvekin_config.prefix}rol ver / al @User @Role\``)] }).sil(5);
            }
        }
        let user = message.mentions.members.first() || message.guild.members.cache.get(args[1]);
        if (!user) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Geçerli Bir User Belirt!**`)] }).sil(5);
        if (user.user.bot) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Bir Bot'a İşlem Uygulayamazsın!**`)]}).sil(5);
        ...
        if (!role.editable) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Belirtilen Role İşlem Yapmaya Yetkim Yetmiyor!**`)] }).sil(5);
        if (args[0] == "al") {
            if (user.roles.cache.has(role.id)) {
                ...
                if (client.kanalbul("rol-log")) client.kanalbul("rol-log").send({
                    embeds: [kanvekin_embed.setDescription(`> **${user} Kullanıcısından ${role} Rolü Alındı!**`).addFields(
                ...
                return message.reply({ embeds: [kanvekin_embed.setDescription(`> **${user} Kullanıcısında ${role} Rolü Mevcut Değil!**`)] }).sil(5);
            }
        }
        if (args[0] == "ver") {
            if (!user.roles.cache.has(role.id)) {
                ...
                db.push(`rollog-${user.id}`,`\`[+]\` ${message.author} (\`${message.author.id}\`)\n*Tarih;* **<t:${Math.floor(Date.now()/1000)}> (<t:${Math.floor(Date.now()/1000)}:R>)**\n*Rol;* ${role} (\`${role.id}\`)`)
                if (client.kanalbul("rol-log")) client.kanalbul("rol-log").send({
                    embeds: [kanvekin_embed.setDescription(`> **${user} Kullanıcısına ${role} Rolü Verildi!**`).addFields(
                ...
                client.false(message)
                return message.reply({ embeds: [kanvekin_embed.setDescription(`> **${user} Kullanıcısında ${role} Rolü Zaten Mevcut!**`)] }).sil(5);
            }
        }
    }
}
