...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        let yt = await db.get("kanvekin-register-staff") || [];
        if(!yt.length > 0)return client.false(message); 

    let kanvekin = message.guild.members.cache.filter(member => member.roles.cache.has(yt[0]) && !member.voice.channel && !member.user.bot).map(member => ("<@" + member.user.id + ">")).join(",");

let kanvekin2 = message.guild.members.cache.filter(member =>member.roles.cache.has(yt[0]) && !member.voice.channel && (member.presence && member.presence.status != "offline") && !member.user.bot).map(member => ("<@" + member.user.id + ">")).join(",");
...
message.reply({embeds:[kanvekin_embed.addFields([
...
const chunk = client.splitMessage(kanvekin, 1500)
const chunk2 = client.splitMessage(kanvekin2, 1500)
