...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        messageUsersData.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => { messageTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${Number(x.topStat).toLocaleString()}` }) })


        voiceUsersData.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => voiceTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.topStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        messageUsersDataDaily.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => messageDailyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${Number(x.dailyStat).toLocaleString()}` }))

        messageUsersDataWeekly.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => messageWeeklyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${Number(x.weeklyStat).toLocaleString()}` }))

        voiceUsersDataDaily.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => voiceDailyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.dailyStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        voiceUsersDataWeekly.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => voiceWeeklyTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.weeklyStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        streamData.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => streamTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.streamStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        cameraData.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).splice(0, 10).map(async (x, index) => cameraTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${moment.duration(x.cameraStat).format("D [Gün], H [sa], m [dk], s [sn]")}` }))

        invites.filter(kanvekin => message.guild.members.cache.get(kanvekin.userId)).slice(0, 10).map(async (x, index) => inviteTop.push({ top: index + 1, avatar: message.guild.members.cache.get(x.userId).user.displayAvatarURL({ extension: "png", forceStatic: true }), tag: message.guild.members.cache.get(x.userId).user.tag, score: `${x.Regular}` }))
/*
...
*/
        const dayvoice = new EmbedBuilder()
            .setImage("attachment://day-voice.png").setColor(`#2f3136`).setTitle(`Günlük Ses Sıralaması`).setURL(`https://discord.gg/82`)

        const daychat = new EmbedBuilder()
            .setImage("attachment://day-chat.png").setColor(`#2f3136`).setTitle(`Günlük Ses Sıralaması`).setURL(`https://discord.gg/82`)

        const weekvoice = new EmbedBuilder()
            .setImage("attachment://week-voice.png").setColor(`#2f3136`).setTitle(`Haftalık Ses Sıralaması`).setURL(`https://discord.gg/82`)

        const weekchat = new EmbedBuilder()
            .setImage("attachment://week-chat.png").setColor(`#2f3136`).setTitle(`Haftalık Ses Sıralaması`).setURL(`https://discord.gg/82`)

        const returnvoice = new EmbedBuilder()
            .setImage("attachment://return-voice.png").setColor(`#2f3136`).setTitle(`Toplam Ses Sıralaması`).setURL(`https://discord.gg/82`)

        const returnmessage = new EmbedBuilder()
            .setImage("attachment://return-message.png").setColor(`#2f3136`).setTitle(`Toplam Mesaj Sıralaması`).setURL(`https://discord.gg/82`)

        const returndavet = new EmbedBuilder()
            .setImage("attachment://return-invite.png").setColor(`#2f3136`).setTitle(`Toplam Davet Sıralaması`).setURL(`https://discord.gg/82`)

        const returnkamera = new EmbedBuilder()
            .setImage("attachment://return-camera.png").setColor(`#2f3136`).setTitle(`Toplam Kamera Sıralaması`).setURL(`https://discord.gg/82`)

        const returnyayın = new EmbedBuilder()
            .setImage("attachment://return-stream.png").setColor(`#2f3136`).setTitle(`Toplam Yayın Sıralaması`).setURL(`https://discord.gg/82`)

        const chanmessage = new EmbedBuilder()
            .setImage("attachment://chan-message.png").setColor(`#2f3136`).setTitle(`Mesaj Kanallarının Sıralaması`).setURL(`https://discord.gg/82`)

        const chanvoice = new EmbedBuilder()
            .setImage("attachment://chan-voice.png").setColor(`#2f3136`).setTitle(`Ses Kanallarının Sıralaması`).setURL(`https://discord.gg/82`)

        //const returnregister = new EmbedBuilder()
          //  .setImage("attachment://return-register.png").setColor(`#2f3136`).setTitle(`Kayıt Sıralaması`).setURL(`https://discord.gg/82`)
        ...
        if (messageDailyTop.length > 0) {
            var messageDailyTopCanvas = await new canvafy.Top()
                ...
                .setBackground("image", kanvekin_config.topArkaplan)
       ...
       // if (registerTop.length > 0 && args[0] && args[0] === "kayıt" || args[0] === "reg" || args[0] === "register" || args[0] === "welcome" || args[0] === "kayıtlar") return mesaj.edit({ content:``,embeds: [returnregister], files: [{ attachment: registerTopCanvas.toBuffer(), name: "return-register.png" }] })
        ...
        mesaj.edit({ content: ``, components: [kanvekin_dropdown], embeds: [kanvekin_embed.setDescription(`> **${client.emoji("emote_hi") !== null ? client.emoji("emote_hi") : ""} Aşşağıdaki Menü Üzerinden Sıralama Verilerini Görebilirsiniz!**\n\n> ${client.emoji("emote_warn") !== null ? client.emoji("emote_warn") : "❓"} *Veya \`.top mesaj,ses,davet,yayın,kamera\` vb. Şekilde Belirtebilirsiniz!*`).setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))] })
        ...
        collector.on('end', async b => {
            ...
            mesaj.edit({ content: `> **Menü Zaman Aşımına Uğradı..**`, components: [kanvekin_dropdown] })
        })
        ...
        collector.on('collect', async b => {
            ...
            if (value == "anasayfa5") {
                b.update({ embeds: [kanvekin_embed], files: [] })
            }
