...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if(tagData.some(tag => member.user.displayName.includes(tag))){
            ...
            if (taggedData && taggedData.Data.includes(member.user.id)) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Bu Üyeye Zaten Daha Önceden Tag Aldırılmış!**")] });
            ...
            let mesaj = await message.reply({ content: member.toString(), components: [buttons], embeds: [kanvekin_embed.setDescription(`> **${message.member} Üyesi Sana Tag Aldırmak İstiyor! Kabul Ediyor Musun?**`)] });
            ...
            await point.updateOne({ guildId: message.guild.id, userId: message.member.id }, { $inc: { point: 15 } }, { upsert: true });
                    kanvekin_embed.setColor("#00ff00").setDescription(`> **${member.toString()} Üyesi Tag Teklifini Kabul Etti!**`);
                    i.update({ embeds: [kanvekin_embed], components: [] });
                   ...
                   message.member.updateTask(message.guild.id, "taglı", 1, message.channel);
                    ...
                    kanvekin_embed.setColor("#ff0000").setDescription(`> **${member.toString()} Kullanıcısı Tag Teklifini Reddedti!**`);
                    i.update({ embeds: [kanvekin_embed], components: [] });
                }
            });
        }else{
          return message.reply({ embeds: [kanvekin_embed.setDescription("> **Üyenin Üzerinde Tag Bulunmamakta!**")] }).sil(5);
        }
