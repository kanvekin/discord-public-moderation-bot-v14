const { PermissionFlagsBits, ButtonStyle, ButtonBuilder, ActionRowBuilder, Events, kanvekin_embedBuilder, StringSelectMenuBuilder, ComponentType, codeBlock, kanvekin_embed } = require("discord.js");
...
module.exports = {
    ...
    const msgMember = message.member;
           ...
           if(!args[0])return message.reply({ embeds: [kanvekin_embed.setDescription("> **Eksik Argüman Belirtmelisin!**\n> *ekle / sil / gönder*")] });
            const member = message.mentions.members.first() || message.guild.members.cache.get(args[1]);
            if (!member) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Bir Kullanıcı Belirtmelisin!**")] });
            if (args[0] === "ekle") {
                ...
                if (!count) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Eklemek İçin Bir Sayı Belirtmelisin!**")] });
                if (!count < 0) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Eklenecek Sayı 0'dan Küçük Olamaz!**")] });
                ...
                if (pointData && client.ranks.some((x) => pointData.point >= x.point && !member.hasRole(x.role))) {
                    ...
                    kanvekin_embed.setColor("#00ff00");
                ...
                message.reply({
                 ...
                 if (!message.member.permissions.has("ADMINISTRATOR")) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **${config.emojis.no} Komutu Kullanmak İçin Yetkin Bulunmamakta!**`)] }).catch((err) => console.log(err), client.tick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
                const count = parseInt(args[2]);
                if (!count) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Çıkarılacak Bir Sayı Belirtmelisin!**")] });
                if (!count < 0) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Çıkarılacak Sayı 0'dan Küçük Olamaz!**")] });
                let pointData = await point.findOne({ guildId: member.guild.id, userId: member.user.id });
                if (!pointData || (pointData && count > pointData.point)) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Çıkarmak İstediğiniz Sayı Kişinin Mevcut Puanından Büyük Olamaz!**")] });
                ...
                if (pointData && client.ranks.some((x) => pointData.point < x.point && member.hasRole(x.role))) {
                    ...
                    kanvekin_embed.setColor("#ff0000");
                    client.kanalbul("rank-log").send({
                        embeds: [
                            kanvekin_embed.setDescription(
                ...
                const count = parseInt(args[2]);
                if (!count) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Puan Vermek İçin Bir Sayı Belirtmelisin!**")] });
                if (!count < 0) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Verilecek Sayı 0'dan Küçük Olamaz!**")] });
                let pointData = await point.findOne({ guildId: member.guild.id, userId: msgMember.user.id });
                if (!pointData || (pointData && count > pointData.point)) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Göndereceğin Puan Kendi Puanından Yüksek Olamaz!**")] });
                ...
                message.reply({ embeds: [kanvekin_embed.setDescription(`> **${member.toString()} Kişisine ${count} Puan Gönderildi!**`)] });
            }else{
    return message.reply({ embeds: [kanvekin_embed.setDescription("> **Eksik Argüman Belirtmelisin!**\n> *ekle / sil / gönder*")] });
