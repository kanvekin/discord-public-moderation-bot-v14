...
module.exports = {
    ...
    kanvekin_embed.setDescription(`> **${member.user.toString()} Kullanıcısının <t:${Math.floor(Date.now()/1000)}:R>ki Verileri;**\n⠀\n`)
    kanvekin_embed.addFields([{name: `${client.emoji("emote_chat") !== null ? client.emoji("emote_chat"):"💭"} • **Toplam Mesaj**`, value: codeBlock("fix",`${messageData ? messageData.topStat : 0} Mesaj`)},
    ...
    kanvekin_embed.addFields([{name:"⠀", value:`⠀`}]);
    kanvekin_embed.addFields([{name:`${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • **Sohbet Ettiğin Chat Kanalları:**\n> **(\`Toplam ${chatLength} Kanal\`)**`,value: `${messageTop}`}]);
    kanvekin_embed.addFields([{name:`${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • **Vakit Geçirdiğin Voice Kanalları:**\n> **(\`Toplam ${voiceLength} Kanal\`)**`,value:`${voiceTop}`}]);
    kanvekin_embed.setFooter({text:`Menü Üzerinden Verilerinize Filtre Uygulayabilirsiniz`,iconURL:member.user.avatarURL({ dynamic: true, size: 2048 })})

    message.reply({ embeds: [kanvekin_embed], components: [kanvekin_dropdown] }).then(b2 => {
            ...
            b2.edit({ content: `> **Menü Zaman Aşımına Uğradı..**`, components: [kanvekin_dropdown] })
            })
            collector.on('collect', async b => {
                if (!b.isStringSelectMenu()) return;
                const value = b.values[0]
                const daily_kanvekin = new EmbedBuilder()
                ...
                .addFields([{name:`${client.emoji("emote_voice") !== null ? client.emoji("emote_voice"):"🔉"} • **Günlük Ses**`,value:codeBlock("fix",`${voiceDaily}`)}]).setColor(`#2f3136`).setTitle(`Günlük Veriler`).setURL(`https://discord.gg/82`).setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))


                const weekly_kanvekin = new EmbedBuilder()
                ...
                .addFields([{name:`${client.emoji("emote_voice") !== null ? client.emoji("emote_voice"):"🔉"} • **Haftalık Ses**`,value:codeBlock("fix",`${voiceWeekly}`)}]).setColor(`#2f3136`).setTitle(`Haftalık Veriler`).setURL(`https://discord.gg/82`).setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))

                const total_kanvekin = new EmbedBuilder()
                .setDescription(`> **${member.user.toString()} Kullanıcısının Haftalık Ses Ve Mesaj Verileri;**`)
        ...
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Public & Sohbet: \`${await category(kanvekin_config.parents.publicParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Kayıt & Welcome: \`${await category(kanvekin_config.parents.registerParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Sorun Çözme & Terapi: \`${await category(kanvekin_config.parents.solvingParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Private & Alone: \`${await category(kanvekin_config.parents.privateParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Oyun & Stream: \`${await category(kanvekin_config.parents.aloneParents)}\`**
        **${client.emoji("emote_value") !== null ? client.emoji("emote_value"):"🔹"} • Konser & Eğlence: \`${await category(kanvekin_config.parents.funParents)}\`**`}]
        ).setColor(`#2f3136`).setTitle(`Kategori Verileri`).setURL(`https://discord.gg/82`).setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))

        const camera_kanvekin = new EmbedBuilder()
                .setDescription(`> **${member.user.toString()} Kullanıcısının Kamera Verileri;**`)
                .addFields([{name:`${client.emoji("emote_camera") !== null ? client.emoji("emote_camera"):"📸"} • **Kamera Verisi**`,value:codeBlock("fix",`${cameraData}`)}]).setColor(`#2f3136`).setTitle(`Kamera Verileri`).setURL(`https://discord.gg/82`).setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))

                const stream_kanvekin = new EmbedBuilder()
                .setDescription(`> **${member.user.toString()} Kullanıcısının Yayın Verileri;**`)
                .addFields([{name:`${client.emoji("emote_computer") !== null ? client.emoji("emote_computer"):"🖥️"} • **Yayın Verisi**`,value:codeBlock("fix",`${streamData}`)}]).setColor(`#2f3136`).setTitle(`Yayın Verileri`).setURL(`https://discord.gg/82`).setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))

                const kayıt_kanvekin = new EmbedBuilder()
                .setDescription(`> **${member.user.toString()} Kullanıcısının Kayıt Verileri;**`)
                .addFields([{name:`${client.emoji("emote_others") !== null ? client.emoji("emote_others"):"🖋️"} • **Kayıt Verisi**`,value:codeBlock("fix",`Erkek; ${erkek}, Kız; ${kadın}, Toplam; ${(erkek+kadın)}`)}]).setColor(`#2f3136`).setTitle(`Kayıt Verileri`).setURL(`https://discord.gg/82`).setThumbnail(member.user.displayAvatarURL({ dynamic: true, size: 2048 }))

                if (value === `anasayfa5`) {
                    await b.update({ embeds: [kanvekin_embed] })
                }

                if (value === `gun`) {
                    await b.update({ embeds: [daily_kanvekin] })
                }

                if (value === `hafta`) {
                    await b.update({ embeds: [weekly_kanvekin] })
                }

                if (value === `toplam`) {
                    await b.update({ embeds: [total_kanvekin] })
                }

                if (value === `kategori`) {
                    await b.update({ embeds: [cat_kanvekin] })
                }

                if (value === `kamera`) {
                    await b.update({ embeds: [camera_kanvekin] })
                }

                if (value === `yayın`) {
                    await b.update({ embeds: [stream_kanvekin] })
                }

                if (value === `kayıt`) {
                    await b.update({ embeds: [kayıt_kanvekin] })
                }
