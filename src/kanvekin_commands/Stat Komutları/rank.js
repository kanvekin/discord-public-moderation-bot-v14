...
module.exports = {
    ...
    const point = args[1];
    if(!args[0])return message.reply({ embeds: [kanvekin_embed.setDescription("> **Bir Argüman Gir**")] }).sil(5);
            if (args[0] === "+") {
                if (!point || isNaN(point)) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Eklenecek Yetkinin Puanını Belirtmelisin!**")] }).sil(5);
                if (client.ranks.some((x) => x.point === point)) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **${point} Puana'a Ulaşıldığında Verilecek Roller Zaten Ayarlı!**`)] }).sil(5);
                const roles = [...message.mentions.roles.values()];
                if (!roles || !roles.length) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Eklenecek Yetkinin rollerini Belirtmelisin!**")] }).sil(5);
                client.rdb.push("ranks", { role: roles.map((x) => x.id), point: parseInt(point) }).sort((a, b) => a.point - b.point);
                return message.reply({ embeds: [kanvekin_embed.setDescription(`> **${point} Puan'a Ulaşıldığında Verilecek Roller:**\n${roles.map((x) => `<@&${x.id}>`).join(", ")}`)] }).sil(5);
            } else if (args[0] === "-") {
                if (!point || isNaN(point)) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Silinecek Yetkinin Puanı'nı Belirtmelisin!**")] }).sil(5);
                if (!client.ranks.some((x) => x.point == point)) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **${point} Puan'a Ulaşıldığında Verilecek Roller Zaten Ayarlanmamışki!**`)] }).sil(5);
                client.rdb.set("ranks",client.ranks.filter((x) => x.point !== point) ).sort((a, b) => a.point - b.point);
                message.reply({ embeds: [kanvekin_embed.setDescription(`> **${point} Puan'a Ulaşıldığında Verilecek Roller Silindi!**`)] }).sil(5);
            } else if (args[0] === "temizle") {
                if (!client.rdb.get("ranks") || !client.rdb.get("ranks").length) return message.reply({ embeds: [kanvekin_embed.setDescription("> **Yetki Yükseltim Sistemi Bomboş!**")] }).sil(5);
                client.rdb.set("ranks", []);
                message.reply({ embeds: [kanvekin_embed.setDescription("> **Tüm Yetkiler Başarıyla Temizlendi!**")] }).sil(5);
            } else if (args[0] === "list") {
