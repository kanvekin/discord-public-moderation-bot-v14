...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
       ...
       if(!args.length) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Yetersiz Argüman Girildi!**`)] }).sil(5);
       if(args.length > 5) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Tek Seferde \`5\` Adet Emoji Ekliyebilirsin!**`)] }).sil(5);
       for(let raw of args){
       ...
       if(parsed.id){
        ...
        message.reply({ embeds: [kanvekin_embed.setThumbnail(emoji.url).setColor("#00ff00").setDescription(`> **\`${emoji.name}\` Adlı Emoji Sunucuya Eklendi!**`)] })
       }
       }
       client.true(message)
