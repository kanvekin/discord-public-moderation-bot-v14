...
module.exports = {
    ...
    execute: async (client, message, args, kanvekin_embed) => {
        ...
        if(!["ekle","çıkar","liste"].some(x=> args[0] == x))return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Bir Argüman Belirtin!**\n\n> \`${kanvekin_config.prefix}özel-komut ekle <komut adı> [verilecek RolID] [verebilecek RolID]\`\n> \`${kanvekin_config.prefix}özel-komut çıkar\`\n> \`${kanvekin_config.prefix}özel-komut liste\``)] }).sil(5);
        if(message.mentions.roles.first())return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Rolleri Etiket İle Değil, ID İle Belirtiniz!**`)] }).sil(5);
        if(args[0] == "ekle"){
        const komutisim = args[1];
        const permRolID = args[2];
        const yetkiRolID = args[3]
        if(!komutisim || !permRolID || !yetkiRolID) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Eksiksiz Girdiğinize Emin Olunuz!**\n\n> \`${kanvekin_config.prefix}özel-komut ekle <komut adı> [verilecek RolID] [verebilecek RolID]\``)] }).sil(5);
        const permsData = db.get(`ozelkomutlar`) || [];  //kanvekin,82 <3
        if(permsData.some(veri=> veri.permName == komutisim)) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **Bu Komut Zaten Daha Önceden Eklenmiş!**`)] }).sil(5);
        if(permsData.length >= 15) return message.reply({ embeds: [kanvekin_embed.setDescription(`> **\`15\`'den Fazla Özel Komut Ekliyemezsin!**`)] }).sil(5);
        db.push(`ozelkomutlar`,{permID:permRolID,permName:komutisim,staffRoleID:yetkiRolID})
        message.reply({embeds:[kanvekin_embed.setDescription(`${codeBlock("md",`# Komut Eklendi!\n< Kullanım: ${kanvekin_config.prefix}${komutisim} <@82/ID>\n> ${komutisim}\n> ${message.guild.roles.cache.get(permRolID).name}\n> ${message.guild.roles.cache.get(yetkiRolID).name} `)}`)]})
        }
        if(args[0] == "çıkar"){
            const permsData = db.get(`ozelkomutlar`) || [];  //kanvekin,82 <3
        ...
        message.channel.send({components:[menu],embeds:[kanvekin_embed.setDescription(`> **Listeden Silmek İstediğiniz Komutu Seçin!**`)]}).then(async msg =>{
            ...
            message.reply({ embeds: [kanvekin_embed.setDescription(`> **\`${liste[index].value}\` Adlı Komut Başarıyla Silindi!**`)]})
            }
            }   
            if(i.values[0] == "iptal") {
